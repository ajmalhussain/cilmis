...
if(!empty($_REQUEST['user_id']) && !empty($_REQUEST['user_warehouse'])){
    
    $user_id    =$_REQUEST['user_id'];
    $wh_id      =$_REQUEST['user_warehouse'];
}
...
$stock_in = $stock_out = $stock_on_hand = $dates_arr = array();
    ...
    $qry_in="
        SELECT
            ...
            sysuser_tab.UserID = $user_id AND
            ...
            <div class="page-content">
                ...
                <div class="row" >
                    ...
                    <div class="col-md-12" >
                        ...
                        <div class="col-md-3" >
                            <button onclick="show_normal()" class="btn btn-sm defualt">Normal Graph</button>
                        </div>
                        
                        <div class="col-md-3" >
                            <button onclick="show_log()" class="btn btn-sm defualt">Adjust on Logarithmic scale</button>
                        </div>
                        
                        <div class="col-md-4" >
                            <button onclick="show_multi()" class="btn btn-sm defualt">Spread on Multi Axis</button>
                        </div>
                    </div>
                </div>
...
//					}]
		...
		function show_normal() {
			...
			window.myLine = new Chart(ctx, config_normal);
		};
		function show_log() {
                        $('#canvas-holder').html('<canvas id="chart-area"></canvas>');
			var ctx = document.getElementById('chart-area').getContext('2d');
			window.myLine = new Chart(ctx, config_log);
		};
		function show_multi() {
                        $('#canvas-holder').html('<canvas id="chart-area"></canvas>');
			var ctx = document.getElementById('chart-area').getContext('2d');
			window.myLine = new Chart(ctx, config_multi);
		};
