...
<html><?php include("menu.php"); ?>
    <h3 align="center">(7) Warehouse Wise Data Comparison of <span style="color:green"> Opening Balance iwth last months closing balance</span> Only Category 1</h3>
    <body>
...
$qry_hf= "SELECT
            ...
            tbl_hf_data.item_id,
            tbl_hf_data.opening_balance AS opening,
            ...
            tbl_warehouse.stkid,
            (SELECT
                    t.closing_balance AS closing
                FROM
                tbl_hf_data t
                WHERE
                t.warehouse_id = tbl_warehouse.wh_id
                and t.item_id  = tbl_hf_data.item_id
                and t.reporting_date = '$date2'
                limit  1
            ) as last_closing
            FROM
                tbl_hf_data
...
<table border="1">
    ...
    foreach($orig_data as $wh_id => $whdata)
    {
        foreach($whdata as $itm_id => $row)
        {
            ...
            $hf_val  = $row['opening'];
            if(!empty($summary_dist_data[$wh_id][$itm_id]['closing']))
                $summ_dist_val = $summary_dist_data[$wh_id][$itm_id]['closing'];
            else
                $summ_dist_val = '';
