...
if ($_POST['ActionType'] == 'Add') {
    ...
    if (strlen($error) > 0) {
        ...
        foreach ($postedArray as $itemid) {
            ...
            $src_stock = isset($_POST['FLDSrcs' . $itemid]) ? $_POST['FLDSrcs' . $itemid] : array();
            ...
            $Removals = isset($_POST['Removals' . $itemid]) ? $_POST['Removals' . $itemid] : 0;
            $DropOuts = isset($_POST['DropOuts' . $itemid]) ? $_POST['DropOuts' . $itemid] : 0;
            $Demand = isset($_POST['Demand' . $itemid]) ? $_POST['Demand' . $itemid] : 0;
            
            $Retrieved = isset($_POST['Retrieved' . $itemid]) ? $_POST['Retrieved' . $itemid] : 0;
            $Change_pos = isset($_POST['Change_pos' . $itemid]) ? $_POST['Change_pos' . $itemid] : 0;
            $Change_neg = isset($_POST['Change_neg' . $itemid]) ? $_POST['Change_neg' . $itemid] : 0;
            ...
            if ($data['num'] == 0) {
                ...
                if(empty($addDate))$addDate=date('Y-m-d H:i:s');
...
//echo $queryadddata;exit;
                $rsadddata = mysql_query($queryadddata) or die(mysql_error());
                $hf_data_id = mysql_insert_id();
                
                
                if (count($src_stock) > 0) {
                    foreach ($src_stock as $key => $val) {
                        $querysrc = "INSERT INTO stock_sources_data(stock_sources_id,hf_data_id,received,created_date)
							Values(							
								" . $key . ",
								" . $hf_data_id . ",
								" . $val . ",
                ...
                if (!empty($hf_type_id) && is_array($hf_type_id)) {
                    ...
                    if ($itemid == '31' || $itemid == '32') {
                        $hf_type_id = $_POST['hf_type_id'];
                        //if ($hf_type_id == 2 || $hf_type_id == 3) {
                            $reffered = $_POST['FLDIsuueUP' . $itemid];
                            $query_reff = "INSERT INTO tbl_hf_data_reffered_by(
                                            tbl_hf_data_reffered_by.hf_data_id,
                                            tbl_hf_data_reffered_by.hf_type_id,
                                            tbl_hf_data_reffered_by.ref_surgeries,
        ...
        $delQry = "DELETE FROM tbl_hf_mother_care_ngo_breakdown WHERE warehouse_id = " . $_POST['wh_id'] . " AND reporting_date='" . $_POST['RptDate'] . "' ";
        //execute query
        mysql_query($delQry) or die(mysql_error());
        
        foreach($_REQUEST['hf_types'] as $type_id => $v){
            
            
                    $qry = "INSERT INTO tbl_hf_mother_care_ngo_breakdown
                                    SET
                                            warehouse_id = $wh_id,
                                            hf_type_id = $type_id,
                                            pre_natal_new = '" .    (isset($_REQUEST['t_pre_natal_new'][$type_id]) ? $_REQUEST['t_pre_natal_new'][$type_id] : 0) . "',
                                            pre_natal_old = '" .    (isset($_REQUEST['t_pre_natal_old'][$type_id])  ? $_REQUEST['t_pre_natal_old'][$type_id] : 0) . "',
                                            post_natal_new = '" .   (isset($_REQUEST['t_post_natal_new'][$type_id]) ? $_REQUEST['t_post_natal_new'][$type_id] : 0) . "',
                                            post_natal_old = '" .   (isset($_REQUEST['t_post_natal_old'][$type_id]) ? $_REQUEST['t_post_natal_old'][$type_id] : 0) . "',
                                            ailment_children = '" . (isset($_REQUEST['t_children_new'][$type_id])   ? $_REQUEST['t_children_new'][$type_id] : 0) . "',
                                            ailment_adults = '" .   (isset($_REQUEST['t_children_old'][$type_id])   ? $_REQUEST['t_children_old'][$type_id] : 0) . "',
                                            general_ailment = '" .  (isset($_REQUEST['t_general_ailment'][$type_id]) ? $_REQUEST['t_general_ailment'][$type_id] : 0) . "', 
                                            referred_implants = '" .  (isset($_REQUEST['t_reffered_implants'][$type_id]) ? $_REQUEST['t_reffered_implants'][$type_id] : 0) . "', 
                                            reporting_date = '" .   (isset($_POST['RptDate']) ? $_POST['RptDate'] : 0) . "' ";
                    //execute query
                    //echo $qry; 
                    mysql_query($qry);
