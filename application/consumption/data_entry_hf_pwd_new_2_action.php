...
if ($_POST['ActionType'] == 'Add') {
    ...
    if (strlen($error) > 0) {
        ...
        foreach ($postedArray as $itemid) {
            ...
            $Demand     = !empty($_POST['Demand' . $itemid]) ? $_POST['Demand' . $itemid] : 0;
            ...
            if ($data['num'] == 0) {
                ...
                $rsadddata = mysql_query($queryadddata) or die(mysql_error().''.$queryadddata);
        ...
        if(!empty($_POST['remarks'])  )
        {
            $qry_r= " INSERT INTO `remarks` (  `wh_id`, `reporting_date`, `user_id`, `remarks`, `module`) VALUES
                        ( '".$_POST['wh_id']."', '".$_POST['RptDate']."','".$_SESSION['user_id']."', '".$_POST['remarks']."', 'consumption_data_entry' ); ";
//            echo $qry_r;exit;
             mysql_query($qry_r) ;
        }
        ...
        if($_SERVER['SERVER_ADDR'] != '::1' && $_SERVER['SERVER_ADDR'] != 'localhost'  && $_SERVER['SERVER_ADDR'] != '127.0.0.1' && $_SERVER['SERVER_NAME'] != 'beta.lmis.gov.pk') 
         {
                ...
                while ($row = mysql_fetch_array($qryRes)) {
                    ...
                    $reporting_date = $row['reporting_date'];
                        ...
                        //this query is taking up huge amount of time to process, due the full table scan in qry used within the func REPgetHFAMC
                        // we need to optimize
