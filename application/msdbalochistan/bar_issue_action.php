...
if (isset($_REQUEST['warehouse']) && !empty($_REQUEST['warehouse'])) {
    ...
    header("Location:bar_issue.php?warehouse=1");
    exit;
}
//check product
...
//check available_qty
if (isset($_REQUEST['ava_qty']) && !empty($_REQUEST['ava_qty'])) {
    //get available_qty
    $ava_qty = str_replace(",", "", $_REQUEST['ava_qty']);
}
