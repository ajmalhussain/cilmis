...
//    $shipment_detail = $objPurchaseOrderProductDetails->get_shipment_by_id($stock_detail['shipment_id']);
    $strSql = " SELECT
        ...
        tbl_stock_master.shipment_id =  '".$stock_detail['shipment_id']."'
            AND tbl_stock_master.po_detail  '".$stock_detail['po_detail']."' ";
    $ress= mysql_query($strSql);
...
$det_id=$sh_id = 0;
$det_id = (!empty($_REQUEST['detail_id'])?$_REQUEST['detail_id']:0);
$sh_id = (!empty($_REQUEST['shipment_id'])?$_REQUEST['shipment_id']:0);
$sql1 = "UPDATE tbl_stock_master SET shipment_id = '".$sh_id."', po_detail = '".$det_id."' WHERE PkStockID=" . $fkStockID;
mysql_query($sql1);
//echo $sql1;exit;
...
header("location:receive_shipment.php?shipment_id=".$sh_id."&detail_id=".$det_id);
exit;
?>