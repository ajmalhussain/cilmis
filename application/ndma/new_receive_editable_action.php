...
alerts_log.response,alerts_log.type,alerts_log.interface ) VALUES ('$to','$cc','$subject','$message','$response','Email','IssuecLMIS')");
    ...
    redirect("new_receive_editable.php");
    exit;
}
//exit;
// End save button
...
//new fields like msd
if (isset($_REQUEST['physical_inspection']))
    $physical_inspection = $_REQUEST['physical_inspection'];
else
    $physical_inspection = '';
//print_r($physical_inspection);exit;
if (isset($_REQUEST['dtl']))
    $dtl = $_REQUEST['dtl'];
else
    $dtl = '';

if (isset($_REQUEST['distribution_plan']))
    $distribution_plan = $_REQUEST['distribution_plan'];
else
    $distribution_plan = '';
//check qty
...
if (isset($_REQUEST['shipment_mode']) && !empty($_REQUEST['shipment_mode'])) {

    $shipment_mode = $_REQUEST['shipment_mode'];
} else {
    $shipment_mode = '';
}
...
//    print_r($_FILES);exit;
    ...
    $uploadFilePath = APP_PATH . 'ndma/attachments/' . $file_name;
//     print_r($uploadFilePath);exit;
    move_uploaded_file($file_tmp, $uploadFilePath);
    $attachment_name = $file_name;
}
else{
    $attachment_name = $_REQUEST['edit_attachment_name'];
}
//set funding source
...
if (empty($trans_no) && $receive_from > 0) {
    ...
    $objStockMaster->source_type = $source_type;
    $objStockMaster->shipment_mode = $shipment_mode;
    $objStockMaster->attachment_name = $attachment_name;
    $objStockMaster->event = $event;
...
//    $objStockMaster->BatchID = $batch_id;
    ...
    if (isset($_REQUEST['edit_voucher_master_id']) && (!empty($_REQUEST['edit_voucher_master_id']))) {
        ...
        $objStockMaster->TranNo = $_REQUEST['receive_no'];
        $objStockMaster->trNo = $_REQUEST['edit_trNo'];
        $objStockMaster->PkStockID = $_REQUEST['edit_voucher_master_id'];
        $objStockMaster->update_whole($_REQUEST['edit_voucher_master_id']);

        $batch_result = $objStockBatch->find_by_batch_no($batch);
//        $row = mysql_fetch_assoc($batch_result);
        if (!empty($batch_result)) {
            $objStockBatch->batch_id = $_REQUEST['edit_hidden'];
            $batch_id=$_REQUEST['edit_hidden'];
        } else {
...
//   print_r($batch_id."..");
   if(empty($batch_id)||($batch_id===true)){
//       print_r("into batch if".$batch_id);
       $batch_id=$_REQUEST['edit_hidden'];
   }
//    print_r("testing batch id".$batch_id);exit;
} 
else { 
    $objStockBatch->Qty = $qty;
    $fkStockID = $stock_id;
...
//        print_r($batch_id);exit;
    } else if ($strDo == 'Update') {
        $objStockDetail->PkDetailID = $_REQUEST['detail_id_hidden'];
        $objStockDetail->BatchID = $batch_id;
        $old_batch_id = $_REQUEST['edit_hidden'];
//        print_r("batch id in update is".$batch_id);exit;
    }
    if (empty($fkStockID)) {
        $fkStockID = $_REQUEST['edit_voucher_master_id'];
    }
...
//    print_r("batch id is".$batch_id);exit;
    ...
    if (!empty($old_batch_id)) {
        $objStockBatch->adjustQtyByWh($old_batch_id, $_SESSION['user_warehouse']);
    } else {
...
if (isset($_REQUEST['edit_voucher_master_id'])) {
    ...
    header("location:new_receive_editable.php");
}
exit;
?>