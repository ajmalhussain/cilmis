...
$qry_itm = "(SELECT
            ...
            ";
if($_SESSION['user_stakeholder1'] == 1 && $_SESSION['user_province1'] == 2){
    $qry_itm .=" OR itminfo_tab.itm_id in (3,13,81) ";
}
$qry_itm .="
            group by 
                itminfo_tab.itm_id
                            ...
                            <div class="widget-body">
                                <div id="export" style="clear:both;margin-top:20px;">
                                    ...
                                    <div style="margin-left:0px !important; width:100% !important;">
                                        <form id="approval_form" method="post" action="clr_all_district_approval2_action.php">
                                                ...
                                                if(!empty($disp_data))
                                                {
                                                    foreach($disp_data as $pk_id => $data)
                                                    {
                                                        ...
                                                        $html.= ' <td style="text-align:center">'.$c.'</td>';
                                                         if($data['wh_id']==25057)
                                                        {
                                                            $html.= ' <td>PPHI Sindh Provincial Store</td>';
                                                        }
                                                        else
                                                        {
...
//                                    echo $qry;exit;            
                                    ...
                                    if($res && mysql_num_rows($res) > 0)
                                    {
                                        ...
                                        while($row = mysql_fetch_assoc($res))
                                        {
                                            ...
                                            if(empty($row['issued']) || $row['issued'] <= 0)
                                            echo ' | <a onClick="return confirm(\'Are you sure, You want to delete this Distribution Plan? This change can Not be undone.\')"  href="distribution_plan_delete.php?plan_id='.$row['pk_id'].'&plan_num='.$row['plan_number'].'&month='.$month.'&year='.$year.'&redirect=clr_all_district_approval2">Delete</a>';
                                            echo '</td>';
                                            echo '</tr>';
                                        }
                                    }
