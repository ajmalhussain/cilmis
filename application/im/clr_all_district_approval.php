...
$qry_itm = "SELECT
            ...
            WHERE
                    ...
                    AND (itminfo_tab.itm_category = 1 OR method_type = 'Syringe')
            ORDER BY
                            ...
                            <div class="widget-body">
                                <div id="printing" style="clear:both;margin-top:20px;">
                                    ...
                                    <div style="margin-left:0px !important; width:100% !important;">
                                        <form id="approval_form" method="post"  action="clr_all_district_approval_action.php">
                                                                ...
                                                                if (!empty($disp_data)) {
                                                                    foreach ($disp_data as $pk_id => $data) {
                                                                        ...
                                                                        if ($data['approval_status'] == 'Prov_Approved')
                                                                            $class1 = 'success';
                                                                        if ($data['approval_status'] == 'Prov_Saved')
                                                                            $class1 = 'warning';
                                                                        ...
                                                                        $html .= ' <a  class="btn btn-xs purple cls_open_remarks" data-id="' . $pk_id . '"   data-name="' . $data['district'] . ' - ' . $data['stkname'] . '">Enter Remarks</a>';
                                                                        $html .= ' </td>';
...
$qry = "SELECT
                                                    ...
                                                    (
                                                            ...
                                                            WHERE
                                                                    ...
                                                                    AND clr_master.approval_status in ( 'Issued','Issue in Process','Hard_Copy_Issued' ) 
                                                    ) AS issued
                                                    FROM
...
if ($res && mysql_num_rows($res) > 0) {
    ...
    while ($row = mysql_fetch_assoc($res)) {
        ...
        echo '<td>'.str_replace('_', ' ', $row['plan_status']).'';
        if(!empty($row['issued']) || $row['issued'] > 0 )
        {
            if($row['issued'] == $row['total_req'] )
                echo ' <b>(Issued)</b> ';
            else
                echo ' (Issuance in progress) ';
        }
        echo '';
        echo '</td>';
        echo '<td>';
        ...
        if(empty($row['issued']) || $row['issued'] <= 0)
        echo ' | <a onClick="return confirm(\'Are you sure, You want to delete this Distribution Plan? This change can Not be undone.\')"  href="distribution_plan_delete.php?plan_id='.$row['pk_id'].'&plan_num='.$row['plan_number'].'&month='.$month.'&year='.$year.'&redirect=clr_all_district_approval">Delete</a>';
        echo '</td>';
        echo '</tr>';
    }
} else {
...
<div id="remarks_modal" class="modal fade" role="dialog">
    <div class="modal-dialog modal-full">
        ...
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="remarks_modal_title">Remarks</h4>
            </div>
            <div id="remarks_modal_body" class="modal-body">
                Processing...
            </div>
            <div class="modal-footer">

                <a id="save_remarks_btn" class="btn btn-success  ">Save</a>            
...
?>
    ...
    $('.cls_open_remarks').click(function () {

        var req_id = $(this).attr('data-id');
        $.ajax({
            url: 'fetch_remarks_screen.php',
            type: 'GET',
            data: 'requisition_id=' + req_id,
            dataType: 'html',
            success: function (data) {
                $('#remarks_modal_body').html('').html(data);
                $('#remarks_modal').modal('show');
            }
        })

    });
    $('#save_remarks_btn').click(function () {
        var master_id = new Array;
        var prod_id = new Array;
        var remarks = new Array;
        $(this).parents('.modal-content').find('.remarks_input').each(function () {

            if (($(this).val() != ''))
            {
//                      console.log($(this).val());
                master_id.push($(this).data("id"));
...
//              console.log(remarks);
        $.ajax({
            ...
            type: "POST",
            url: 'ajax_insert_remarks.php',
            data: {master_id: master_id, prod_id: prod_id, remarks: remarks},
            dataType: 'html',
            success: function (data) {
                console.log(data);
                if(data==1)
                {
                toastr.success('Your Remarks Saved');
