...
//                print_r($qry);exit;
                ...
                <table class='table table-bordered table-condensed'>
                    <?php while ($row = mysql_fetch_assoc($qryRes)) { ?>
                        ...
                        <tr><th>Quantity</th>
                            <td class="bold"><?php echo number_format($row['Qty']) ?>
                                <span class="pull-right">
                                    <a href="<?=APP_URL?>developer_reports/fix_stock_batch_redirect.php?id=<?=$batch_id?>&redirect=im/product-ledger-history">*</a>
                                </span>
                            </td></tr>
                ...
                $qry_trail = "SELECT tbl_stock_master.WHIDFrom,
                               ...
                               LEFT JOIN tbl_warehouse ON tbl_stock_master.WHIDFrom = tbl_warehouse.wh_id
                               LEFT JOIN tbl_warehouse AS tt ON tbl_stock_master.WHIDTo = tt.wh_id
                               LEFT JOIN stakeholder ON tt.stkid = stakeholder.stkid
                               LEFT JOIN tbl_locations ON tt.prov_id = tbl_locations.PkLocID
LEFT JOIN tbl_trans_type ON tbl_stock_detail.adjustmentType = tbl_trans_type.trans_id
                               WHERE stock_batch.batch_id='$batch_id'
                               ORDER BY
