...
";
                                    ...
                                    while ($row = mysql_fetch_assoc($res)) {
                                        if (!empty($_SESSION['user_province1']) && $_SESSION['user_province1'] != 2) {
                                            if ($row['itm_id'] == 34)
                                                continue;
                                        }
                                        $itm_name_id[$row['itm_name']] = $row['itm_id'];
                                    ...
                                    <div id="printing" style="clear:both;margin-top:20px;">
                                        <div style="margin-left:0px !important; width:100% !important;">
                                            ...
                                            <table width="200" id="headerTable" align="right">
                                                ...
                                                <tr>
                                                    <td>
                                                        <p style="width: 100%; display: table;"> <span style="display: table-cell; width: 84px;">Requisition Date: </span> <span style="display: table-cell; border-bottom: 1px solid black;"><?php echo date('d/m/Y'); ?></span> </p>
                                                    </td>
                                                </tr>
                                            </table>
                                            ...
                                            <form name="frm" id="frm" method="post" action="">
                                                <table width="100%" id="myTable" cellspacing="0">
                                                    ...
                                                    <?php
                                                     $colNum = 1;
                                                    foreach ($itm_name_id as $pname => $itm) {
                                                    ?>
                                                    <tr>
                                                        <td><?php echo $colNum; ?></td>
                                                        <td  class="td_chk" data-itm-id="<?php echo $itm; ?>"  style="text-align:center"> <input type="checkbox" checked="checked" class="prod_chk" id="chk_<?php echo $itm; ?>" name="chk[<?php echo $itm; ?>]" value="<?php echo $itm; ?>"><br><?php echo $pname; ?></td>
                                                        <?php
                                                        // A-1
                                                    ...
                                                    ?>
                                                        ...
                                                        <?php
                                                        // A-4
                                                        if (empty($SOHFieldArr[$itm]))    $SOHFieldArr[$itm] = 0;
                                                        if (empty($SOHDistrictArr[$itm])) $SOHDistrictArr[$itm] = 0;

                                                        echo "<td class=\"TAR td_chk\" data-itm-id=\"$itm\">" . ((strlen($SOHFieldArr[$itm]) > 0) ? number_format($SOHFieldArr[$itm] + $SOHDistrictArr[$itm]) : '') . "</td>";
                                                        ?>
                                                        <input type="hidden" name="total_stock[]" value="<?php echo $SOHFieldArr[$itm] + $SOHDistrictArr[$itm]; ?>" />
                                                        <?php
                                                        // A-5
                                                        $des_two_q = (!empty($consumptionArr[$itm]) ? $consumptionArr[$itm] : '');
                                                                echo "<td class=\"TAR td_chk\" data-itm-id=\"$itm\">" . ((strlen($des_two_q) > 0) ? number_format($des_two_q * 2) : '') . "</td>";
                                                            ?>
                                                                <input type="hidden" name="desired_stock[]" value="<?php echo $des_two_q * 2; ?>" />
                                                            <?php
                                                            // A-6 
                                                            $cons_a6 = (!empty($consumptionArr[$itm]) ? $consumptionArr[$itm] : '0');
                                                                $a6 = ($cons_a6 * 2) - ($SOHFieldArr[$itm] + $SOHDistrictArr[$itm]);
                                                                $a6 = ($a6 > 0) ? $a6 : 0;
                                                                echo "<td class=\"TAR td_chk\" data-itm-id=\"$itm\">" . ((strlen($cons_a6) > 0) ? number_format($a6) : '') . "</td>";
                                                            ?>
                                                                <input type="hidden" name="replenishment[]" value="<?php echo $a6; ?>" />
                                                           
                                                           <?php // A-7 
                                                           $cons_a7 = (!empty($consumptionArr[$itm]) ? $consumptionArr[$itm] : '0');
                                                           $a6 = ($cons_a7 * 2) - ($SOHFieldArr[$itm] + $SOHDistrictArr[$itm]);
                                                           $a6 = ($a6 > 0) ? $a6 : 0;
                                                           echo '<td class="TAR td_chk\" data-itm-id=\"$itm\"><input type="" style="font-size: 11px;padding:1px 1px !important;" class="form-control input-sm qty cls_print_input" data-id="' . $itm . '" step="1" min="0" name="quantity_requested[]" value="' . $a6 . '" data-orig-val="' . $a6 . '" /></td>';

                                                           // A-8 
                                                           echo '<td class="TAR td_chk cls_print_input\" data-itm-id=\"$itm\"><textarea rows="2" cols="10"  maxlength="500" class="form-control  input-sm remarks_box" data-id="' . $itm . '" name="remarks[]"></textarea><span id="msg_' . $itm . '" class="red"></span></td>';
