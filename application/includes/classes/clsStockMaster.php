...
class clsStockMaster {
    ...
    //db fileds 
    protected static $db_fields = array('TranDate', 'TranNo', 'TranTypeID', 'TranRef', 'WHIDFrom', 'WHIDFromSupplier', 'WHIDTo', 'CreatedBy', 'CreatedOn', 'ReceivedRemarks', 'temp', 'trNo', 'LinkedTr', 'issued_by', 'source_type', 'shipment_mode', 'attachment_name', 'requisition_type', 'method', 'event', 'po_detail');
    //pk stock id
    public $PkStockID;
    //transaction date
    public $TranDate;
    ...
    protected function attributes() {
        ...
        foreach (static::$db_fields as $field) {
            if (property_exists($this, $field)) {
                if ($field == 'po_detail' && (!isset($this->$field) || $this->$field == '' || empty($this->$field))) {
                    continue;
                }
...
//echo 'upd:::'.$sql;exit;
    ...
    public function update_whole($master_id) {
        ...
        $sql .= " WHERE PkStockID=" . $database->escape_value($master_id);
//        print_r($sql);exit;
        $database->query($sql);
        return ($database->affected_rows() == 1) ? true : false;
    }
