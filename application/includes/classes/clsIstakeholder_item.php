...
class clsstakeholderitem {
	...
	if(!empty($this->unit_price)) $strSql .= "		,unit_price = '" . $this->unit_price . "' ";
        //query result
//        echo $strSql;exit;
        $rsSql = mysql_query($strSql) or die("Error Addstakeholderitem");
        if (mysql_affected_rows()) {
...
//        echo $strSql;exit;
    ...
    function Editstkholderitem() {
        //edit query
        //
        //// DELETE is removed. because manufacturers were being removed .... 
        ...
        while ($stk_n != 0) {
            ...
            $strSql2 = " SELECT stk_id as pk_id FROM  stakeholder_item WHERE stkid = '".$this->m_stkid[$no]."' AND stk_item='" . $this->m_stk_item."' ";
            $rsSql2 = mysql_query($strSql2) or die("Error in select stk items");
            $row_pk = mysql_fetch_array($rsSql2);
//            echo $row_pk['pk_id'];
//            exit;
            if(!isset($row_pk['pk_id']))
            {
                //insert only when data doesnt exist before.
                $strSql = " INSERT INTO stakeholder_item(stkid,stk_item) VALUES(" . $this->m_stkid[$no] . "," . $this->m_stk_item . ") ";
                //query result
                $rsSql = mysql_query($strSql) or die("Error Addstakeholderitem");
            }
