...
class clsShipments {
    ...
    //db fileds
    protected static $db_fields = array('reference_number', 'item_id', 'manufacturer', 'shipment_date', 'shipment_quantity', 'stk_id', 'procured_by', 'status', 'created_date', 'created_by', 'modified_by','wh_id','unit_price', 'po_number', 'po_date');
    //pk stock id
...
//echo $sql;exit;
    ...
    function NDMAShipmentSearch($type, $wh_id, $groupby = '', $page_type = '') {
        ...
        $strSql = "SELECT
		    ...
		    tbl_itemunits.UnitType,
                    ...
                    LEFT JOIN tbl_warehouse ON shipments.stk_id = tbl_warehouse.wh_id
    ...
    function ShipmentSearch($type, $wh_id, $groupby = '', $page_type = '') {
        ...
        $strSql = "SELECT
                    ...
                    LEFT JOIN tbl_stock_master ON tbl_stock_master.shipment_id = shipments.pk_id
                    LEFT JOIN tbl_stock_detail ON tbl_stock_detail.fkStockID = tbl_stock_master.PkStockID
                    LEFT JOIN stakeholder_item ON shipments.manufacturer = stakeholder_item.stk_id
                    LEFT JOIN stakeholder ON stakeholder_item.stkid = stakeholder.stkid
                    ";
         if(isset($_SESSION['user_warehouse']))
            {
                $where[] = " shipments.wh_id = ".$_SESSION['user_warehouse']." ";
            }
        if(isset($_SESSION['user_level']) && $_SESSION['user_level'] == 2)
        {
            $strSql.= " INNER JOIN funding_stk_prov ON shipments.stk_id = funding_stk_prov.funding_source_id ";
        ...
        if (!empty($this->item_id)) {
            if(is_array($this->item_id)){
                ...
                $where[] = "shipments.item_id in  (" .implode(',',$this->item_id)  . ") ";
            }
            else{
                $where[] = "shipments.item_id = '" . $this->item_id . "'";
            }
