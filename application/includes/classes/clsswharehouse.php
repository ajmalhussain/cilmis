...
//npkid
    ...
    function Addwarehouse() {
        ...
        if (($this->im_date == '')||$this->im_date=='-01') {
            $this->im_date = '0000-00-00';
        }
        if (empty($this->hf_type_id) || $this->hf_type_id == '') {
            $this->hf_type_id = 0;
        }
        //echo '<pre>';print_r($this);
        $strSql = "INSERT INTO tbl_warehouse (stkid,stkofficeid,prov_id,dist_id,wh_name,locid, hf_type_id, reporting_start_month, editable_data_entry_months, is_lock_data_entry, wh_rank, dhis_code, is_active, is_allowed_im,created_by,modified_by,modified_date,im_start_month,parent_id) VALUES(" . $this->m_stkid . "," . $this->m_stkofficeid . "," . $this->m_prov_id . "," . $this->m_dist_id . ",'" . $this->m_wh_name . "'," . $this->m_dist_id . ",'" . $this->hf_type_id . "','" . $this->reporting_start_month . "','" . $this->editable_data_entry_months . "','" . $this->is_lock_data_entry . "','" . $this->wh_rank . "','" . $this->hf_code . "','" . $this->is_active . "','" . $this->is_allowed_im . "','".$this->user_id."','".$this->user_id."',now()," .(!empty($this->im_date)? "'".$this->im_date."'":'NULL') . ",'".$this->parent_id."')";
        //echo $strSql;exit;
    ...
    function Editwarehouse() {
        ...
        if($this->im_date == '-01') $this->im_date = '';
        $im_date = ",im_start_month= " . (!empty($this->im_date)?"'".$this->im_date."'":NULL) . " ";
        if ($this->im_date != '') {
            $strSql .= $im_date;
        }
        $modified = ",modified_by='" . $this->user_id. "' , modified_date = now() ";
        if (!empty($this->user_id)) {
            $strSql .= $modified;
        } else {
            $strSql .= ',modified_by=1 , modified_date = now()';
        }
...
//        echo $strSql;exit;
    ...
    function Deletewarehouse() {
        ...
        // NOTE :::: Delete functionality disabled from system for security reasons.You may deactivate the warehouse.
...
//        }
    ...
    function get_funding_sources_of_province($prov_id=null) {
        if($prov_id==6){
            $prov_id=3;
        }
        $strSql = "SELECT
			...
			WHERE
				stakeholder.stk_type_id = 2
			AND tbl_warehouse.is_active = 1 ";
        if(!empty($prov_id)) $strSql .= " AND (tbl_warehouse.prov_id = '$prov_id')  ";
        $strSql .= "
			ORDER BY
				stakeholder.stkorder ASC";
    ...
    function get_supplier_of_province($prov_id=null) {
        if($prov_id==6){
            $prov_id=3;
        }
        $strSql = "SELECT
    ...
    function get_assigned_warehouses($user_id) {
        $strSql = "SELECT
                        wh_user.wh_id
                    FROM
                        wh_user
                    ...
                    ";
        //echo $strSql;
        $qryRes = mysql_query($strSql);
        $result = array();
        while($row = mysql_fetch_array($qryRes))
        {
            $result[$row['wh_id']] = $row['wh_id'];
        }
        return $result;
    }
    /**
...
//select query
    ...
    function get_wards_of_district($hospital_id) {
        //select query
        //gets
        //Health Facility Warehouses of District
        $strSql = "SELECT
				...
				FROM
					(
						...
						AND tbl_warehouse.stkid = tbl_hf_type_rank.stakeholder_id
						WHERE
							tbl_warehouse.parent_id = " . $hospital_id. "
						AND stakeholder.lvl = 9 AND
                                                tbl_warehouse.is_active = 1
					) A
				GROUP BY
					A.wh_id
    ...
    function GetHealthFacilityWarehousesofDistrictIRMNCH($distId, $mainstk) {
        //select query
        //gets
        //Health Facility Warehouses of District
        $strSql = "SELECT
