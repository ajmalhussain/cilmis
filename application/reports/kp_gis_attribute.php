...
//select query
    // Get district name
    $filter = '';
    if ($districtId != '' || !empty($districtId)) {
        $filter = "  WHERE
...
if (isset($_POST['submit'])) {
    $filter = '';
    if ($districtId != '' || !empty($districtId)) {
        $filter = "  AND tbl_warehouse.dist_id = $districtId";
    }
    //select query
    //gets
    //warehouse id
...
//                            print_r( $total);exit;
        ...
        if ($selProv == 3) {
                                            ...
                                            <table id="myTable" cellspacing="0" align="center">
                                                <thead>
                                                    <tr>
                                                        ...
                                                        <th colspan="<?php echo count($items) ?>">Consumption</th>
            <?php
            foreach ($items as $name) {
            ...
            foreach ($whName as $whId => $whName) {
                ...
                if ($whType[$whId] != $hfTypeId) {
                    ...
                    if ($hfTypeCount > 1 && $print == true && $last_wh_type == 1 && $hfTypeId != 1) {
                        ...
                        foreach ($items as $methodName) {
                            echo "<th class=\"right\">" . ( (array_sum($subTotal[$methodName]['issue_balance']) != 0) ? number_format(array_sum($subTotal[$methodName]['issue_balance'])) : 0 ) . "</th>";
                        }
                        foreach ($items as $methodName) {
                ...
                <?php
                foreach ($items as $methodName) {
                    echo "<td class=\"right\">" . ( ($data[$whId][$methodName]['issue_balance'] != 0) ? number_format($data[$whId][$methodName]['issue_balance']) : 0 ) . "</td>";
                    $subTotal[$methodName]['issue_balance'][] = $data[$whId][$methodName]['issue_balance'];
                    $gTotal[$methodName]['issue_balance'] += $data[$whId][$methodName]['issue_balance'];
                }
            ...
            <?php
            foreach ($items as $methodName) {
                echo "<th class=\"right\">" . ( (($gTotal[$methodName]['issue_balance']) != 0) ? number_format(($gTotal[$methodName]['issue_balance'])) : 0 ) . "</th>";
            }
