                                        ...
                                        if (!empty($_REQUEST['month'])) {
                                            ...
                                            $q_all = "SELECT
                                                    ...
                                                    WHERE
                                                            ...
                                                            tbl_warehouse.stkid = '".$ussd_stk."'
                                                    ";
                                            if (!empty($_REQUEST['dist_id'])) {
                                                $q_all .= "  AND tbl_warehouse.dist_id = '" . $_REQUEST['dist_id'] . "' ";
                                            }
                                            $q_all .= "  ORDER BY tbl_locations.LocName,tbl_warehouse.wh_name ";
                                            $wh_arr = $dist_count = $dist_wh_arr = $dist_names = array();
                                            ...
                                            while ($row = mysql_fetch_array($rsp)) {
                                                ...
                                                $dist_wh_arr[$row['dist_id']][$row['wh_id']] = $row['wh_name'];
                                                $dist_names[$row['dist_id']] = $row['dist_name'];
                                                
                                                @$dist_count[$row['dist_name']]++;
                                            }
...
//                                        }
                                            ...
                                            $last_dist = '';
                                            $dist_totals= $prov_totals= $dist_percentages =  array();
                                            foreach ($dist_wh_arr as $dist_id => $dist_data) {
                                                $dist_name = $dist_names[$dist_id];
                                                foreach ($dist_data as $whid => $wh_name) {
                                                            if(!empty($_REQUEST['display_id']) && $_REQUEST['display_id']=='reported_only' && !isset($reported_arr[$whid]))
                                                            {
                                                                continue;
                                                            }
                                                            echo '<tr>';
                                                            if($dist_name != $last_dist){

                                                                    echo '<td class="h4" rowspan="'.($dist_count[$dist_name]+2).'">' . $dist_name . '</td>';
                                                                    $last_dist = $dist_name;
                                                            }
                                                            ...
                                                            foreach ($weeks_arr as $st => $end) {
                                                                ...
                                                                if (!empty($reported_arr[$whid][$st]) && $reported_arr[$whid][$st] == '1') {
                                                                    ...
                                                                    @$dist_totals['reported'][$dist_id][$st] ++;
                                                                    @$prov_totals['reported'][$st] ++;
                                                                } else {
                                                                    echo '<i class="fa fa-times" style="font-size:15px;color:red !important;"></i>';
                                                                }
                                                                echo '</td>';

                                                                    @$dist_totals['all'][$dist_id][$st] ++;
                                                                    @$prov_totals['all'][$st] ++;
                                                            }
                                                            echo '</tr>';
                                                }//end of wh foreach
                                                ...
                                                echo '<tr class="info">';
                                                echo '<td rowspan="2" colspan="2">District '.$dist_name.' Total:</td>';
                                                    foreach ($weeks_arr as $st => $end) {
                                                            echo '<td style="text-align:center;">';
                                                                echo (!empty($dist_totals['reported'][$dist_id][$st])?$dist_totals['reported'][$dist_id][$st]:0);
                                                                echo ' / ';
                                                                echo $dist_totals['all'][$dist_id][$st];
                                                            echo '</td>';
                                                        }
                                                echo '</tr>';
                                                echo '<tr class="info">';
                                                    foreach ($weeks_arr as $st => $end) {
                                                            echo '<td style="text-align:center;">';
                                                                $perc = 0 ;
                                                                if(!empty($dist_totals['reported'][$dist_id][$st]) && !empty($dist_totals['all'][$dist_id][$st]) && $dist_totals['all'][$dist_id][$st] > 0)
                                                                {
                                                                    $perc = 100 * $dist_totals['reported'][$dist_id][$st] / $dist_totals['all'][$dist_id][$st] ;
                                                                }
                                                                echo number_format($perc,1).' %';
                                                            echo '</td>';
                                                            
                                                            $dist_percentages[$dist_id] = $perc;
                                                        }
                                                echo '</tr>';
                                             ...
                                             echo '<tr class="success h4">';
                                                echo '<td colspan="3">Provincial Total</td>';
                                                    foreach ($weeks_arr as $st => $end) {
                                                            echo '<td style="text-align:center;">';
                                                                $perc = 0 ;
                                                                if(!empty($prov_totals['reported'][$st]) && !empty($prov_totals['all'][$st]) && $prov_totals['all'][$st] > 0)
                                                                {
                                                                    $perc = 100 * $prov_totals['reported'][$st] / $prov_totals['all'][$st] ;
                                                                }
                                                                echo number_format($perc,1).' %';
                                                            echo '</td>';
...
<script type="text/javascript">
    $(function () {
        ...
        $('#dist_id').change(function (e) {
            ...
            $.ajax({
                url: 'ajax_calls.php',
                data: {dist_id: $(this).val(), show_what: 'sdps', stk_id: '<?= $ussd_stk ?>'},
                type: 'POST',
