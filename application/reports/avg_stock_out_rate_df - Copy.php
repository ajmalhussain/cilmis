...
WHERE
tbl_hf_data.reporting_date BETWEEN '$from_date_dorment' AND '$to_date_dorment'
...
$dorment_facility = $dorment_facility_list = array();
while ($row = mysql_fetch_assoc($res)) {
    $dorment_facility_list[$row['wh_id']] = $row['wh_id'];
...
//query result
                ...
                if (isset($_REQUEST['submit_btn'])) {
                    ...
                    $disabled_qry = "
                                                SELECT
                                                    GROUP_CONCAT(DISTINCT warehouse_status_history.warehouse_id) whids,
                                                ...
                                                AND tbl_warehouse.wh_id NOT IN (".implode(',',$dorment_facility_list).")
                                                $where_clause
                                                GROUP BY
                                                        tbl_warehouse.prov_id,
                    ...
                    while ($row_d = mysql_fetch_array($res_d)) {
                        $disabled_facility_list[] = $row_d['whids'];
                        $disabled_count[$row_d['prov_id']][$row_d['stk_item']][$row_d['reporting_month']] = $row_d['cnt'];
                        if (empty($disabled_count['all'][$row_d['stk_item']][$row_d['reporting_month']]))
                            $disabled_count['all'][$row_d['stk_item']][$row_d['reporting_month']] = 0;
                    ...
                    $qry_1 = "  
                    SELECT
                    stats_tab_reporting_rate.province_id prov_id,
                stakeholder_item.stk_item as itm,
                    COUNT(
                        stats_tab_reporting_rate.warehouse_id
                    ) totalWH
                FROM
                    stats_tab_reporting_rate
                INNER JOIN tbl_warehouse ON stats_tab_reporting_rate.warehouse_id = tbl_warehouse.wh_id
                INNER JOIN stakeholder_item ON tbl_warehouse.stkid = stakeholder_item.stkid
                INNER JOIN stakeholder ON tbl_warehouse.stkofficeid = stakeholder.stkid
                WHERE
                   stats_tab_reporting_rate.reporting_month BETWEEN '" . $from_date . "' and '" . $to_date . "'
                   AND tbl_warehouse.wh_id NOT IN (".implode(',',$dorment_facility_list).")
                   AND tbl_warehouse.wh_id NOT IN (".implode(',',$disabled_facility_list).")
                $where_clause
                AND
stakeholder.lvl = 7
                GROUP BY
                    stats_tab_reporting_rate.province_id,
                    stakeholder_item.stk_item
                                            ";
                    //echo $qry_1;exit;
                    $res_1 = mysql_query($qry_1);
                    ...
                    $q_reporting = "SELECT
                                                            ...
                                                            WHERE
                                                                ...
                                                                /*AND NOT (tbl_warehouse.prov_id = 1 and tbl_warehouse.stkid = 73)*/
                    ...
                    while ($row = mysql_fetch_assoc($res_reporting)) {
                        ...
                        if (empty($reporting_wh_arr['all'][$row['item_id']][$row['reporting_date']]))
                            $reporting_wh_arr['all'][$row['item_id']][$row['reporting_date']] = 0;
                        if (empty($reporting_wh_arr[$row['prov_id']][$row['item_id']][$row['reporting_date']]))
                            $reporting_wh_arr[$row['prov_id']][$row['item_id']][$row['reporting_date']] = 0;
...
//echo '<pre>';print_r($reporting_wh_arr);print_r($reporting_wh_arr2);exit;    
                    ...
                    while ($row = mysql_fetch_assoc($qryRes)) {
                        ...
                        if (empty($so_arr['all'][$row['item_id']][$rep_month]))
                            $so_arr['all'][$row['item_id']][$rep_month] = 0;
                        if (empty($so_arr[$row['prov_id']][$row['item_id']][$rep_month]))
                            $so_arr[$row['prov_id']][$row['item_id']][$rep_month] = 0;
                                                ...
                                                foreach ($so_arr as $prov_id => $prov_data) {
                                                    ...
                                                    foreach ($itm_arr as $itm_id => $itm_name) {
                                                        ...
                                                        foreach ($months_list as $k => $v) {
                                                            ...
                                                            $master_total = $total_sdps[$prov_id][$itm_id]+$disabled_fac+$dorment_facility[$prov_id][$itm_id];
                                                            //$to_be_reported = $master_total - $disabled_fac;
                                                            $to_be_reported = $total_sdps[$prov_id][$itm_id];

                                                            $val = (isset($prov_data[$itm_id][$v]) ? $prov_data[$itm_id][$v] : 0);
                                                            
                                                            // Dorment Facility Check
                                                            /*if (isset($dorment_facility[$prov_id][$itm_id]) && !empty($dorment_facility[$prov_id][$itm_id])) {
                                                                $val = $val - $dorment_facility[$prov_id][$itm_id];
                                                                $to_be_reported = $to_be_reported - $dorment_facility[$prov_id][$itm_id];
                                                                $reporting_wh_arr[$prov_id][$itm_id][$v] = $reporting_wh_arr[$prov_id][$itm_id][$v]-$dorment_facility[$prov_id][$itm_id];
                                                            }*/
                                                            ...
                                                            echo '<td align="right" class="total_n" title="All:' . $master_total . ',Disabled:' . $disabled_fac . ',Dormant:' . $dorment_facility[$prov_id][$itm_id] . '">' . number_format($to_be_reported) . '</td>';
