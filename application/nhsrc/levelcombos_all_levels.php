...
if ($_SESSION['user_id'] == 10201) {
    $stk_where = " AND stakeholder.stkid in (1053) ";
} else if ($_SESSION['user_warehouse'] == 96585) {
    $stk_where = " AND stakeholder.stkid in (1222) ";
} else if ($_SESSION['user_warehouse'] == 96584) {
    $stk_where = " AND stakeholder.stkid in (1192) ";
} else {
    $stkid = $_SESSION['user_stakeholder'];
    $stk_where = " AND stakeholder.stkid in ($stkid) ";
}
...
switch ($user_lvl) {
    case 1:
        if ($_SESSION['user_id'] == 10201) {
            $arrayProv = array(
                '7' => 'Influenza Sentinel Site',
                '4' => 'PPHL',
                '3' => 'NIH Departments',
                '8' => 'Individuals'
            );
        } else if ($_SESSION['user_stakeholder'] == 1222) {
            $arrayProv = array(
                '1' => 'National',
                '2' => 'Province/Region/Federating Area',
                '3' => 'District',
                '4' => 'Field/Tehsil/Town',
                '7' => 'Health Facility'
            );
