...
//    print_r($queryItem);exit;
    ...
    $caption = 'Lakki Marwat District Stock Status (Drilldown)';
}  
...
//        print_r($stock_arr);exit;
        ...
        foreach ($wh_name as $wh_id => $name) {
            ...
            $xmlstore .= '    <set  value="' . $stock_arr[$wh_id]['receive'] . '" link="P-detailsWin,width=900,height=600,toolbar=no,scrollbars=yes, resizable=no-dashboard_wms_dashlet_dist_i3_table.php?prov_sel='.$province.'&product='.$product.'&from_date='.$from_date.'&to_date='.$to_date.'&search_type='.$search_type.'"  />';
        }
        $xmlstore .= '  </dataset>';
        $xmlstore .= ' <dataset   seriesname="Consumption">';
        foreach ($wh_name as $wh_id => $name) {
            $xmlstore .= '    <set  value="' . $stock_arr[$wh_id]['consumption'] . '"  link="P-detailsWin,width=900,height=600,toolbar=no,scrollbars=yes, resizable=no-dashboard_wms_dashlet_dist_i3_table.php?prov_sel='.$province.'&product='.$product.'&from_date='.$from_date.'&to_date='.$to_date.'&search_type='.$search_type.'"/>';
        }
        $xmlstore .= '  </dataset>';
        $xmlstore .= ' <dataset   seriesname="AMC">';
        foreach ($wh_name as $wh_id => $name) {
            $xmlstore .= '    <set  value="' . $stock_arr[$wh_id]['amc'] . '"  link="P-detailsWin,width=900,height=600,toolbar=no,scrollbars=yes, resizable=no-dashboard_wms_dashlet_dist_i3_table.php?prov_sel='.$province.'&product='.$product.'&from_date='.$from_date.'&to_date='.$to_date.'&search_type='.$search_type.'"/>';
        }
        $xmlstore .= '  </dataset>';
        $xmlstore .= ' <dataset   seriesname="SOH">';
        foreach ($wh_name as $wh_id => $name) {
            $xmlstore .= '    <set  value="' . $stock_arr[$wh_id]['soh'] . '"  link="P-detailsWin,width=900,height=600,toolbar=no,scrollbars=yes, resizable=no-dashboard_wms_dashlet_dist_i3_table.php?prov_sel='.$province.'&product='.$product.'&from_date='.$from_date.'&to_date='.$to_date.'&search_type='.$search_type.'"/>';
        }
        $xmlstore .= '  </dataset>';
...
echo renderChart(PUBLIC_URL . "FusionCharts/Charts/MSColumn2D.swf", "", $xmlstore, 'dist_im_a3', '100%', 300, false, false);
?>
