...
$wh_id = (!empty($_REQUEST['wh_id']) ? $_REQUEST['wh_id'] : '');
$level = (!empty($_REQUEST['level']) ? $_REQUEST['level'] : '');
?>
<div class="widget widget-tabs" style="border:0px;">
    <div class="widget-body" id="dd" style='background-color: white;'>

        <h3 style="text-align: center;">Product Wise Drilldown</h3>
        <?php
...
//exit;
        ...
        while ($row = mysql_fetch_assoc($qryRes)) {
            $stock_arr[$row['wh_id']][$row['item_id']]['soh'] = $row['soh'];
...
//        print_r($qry);exit;
        ...
        while ($row = $dist_res->fetch_assoc()) {
            $stock_arr[$row['wh_id']][$row['item_id']]['consumption'] = $row['Issue'];
        }
...
FROM
	(
		...
		AND tbl_stock_master.TranTypeID = 2
                AND tbl_warehouse.wh_id = $wh_id
                AND stakeholder.lvl = $level 
		AND stock_batch.item_id IN ($product)
...
//print_r($qry_avg_consumption);exit;
        ...
        while ($row = $dist_res->fetch_assoc()) {
            $stock_arr[$row['wh_id']][$row['item_id']]['avg_consumption'] = $row['AMC'];
        }
        foreach ($itm_arr as $key => $value) {
            $stock_arr[$wh_id][$key]['mos'] = ($stock_arr[$wh_id][$key]['soh'] / $stock_arr[$wh_id][$key]['avg_consumption']);
        }
//echo '<pre>';
...
//        $xmlstore .= '  </dataset>';
        ...
        foreach ($itm_arr as $id => $name) {
            $xmlstore .= '<set  value="' . number_format($stock_arr[$wh_id][$id]['mos'], 2) . '"  />';
        }
        $xmlstore .= '  </dataset>';
        $xmlstore .= ' </chart>';
//        print_r($xmlstore);exit;
        FC_SetRenderer('javascript');
        echo renderChart(PUBLIC_URL . "FusionCharts/Charts/MSColumn2D.swf", "", $xmlstore, 'drilldown', '100%', 300, false, false);
        ?>
