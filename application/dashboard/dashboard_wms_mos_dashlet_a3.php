...
while ($row = $dist_res->fetch_assoc()) {
     $stock_arr[$row['wh_id']]['consumption'] += $row['Issue'];
}
...
while ($row = $dist_res->fetch_assoc()) {
     $stock_arr[$row['wh_id']]['avg_consumption'] += $row['AMC'];
}
...
//        $xmlstore .= '  </dataset>';
       ...
       foreach ($wh_arr as $id => $name) {
            $xmlstore .= "<set  value='". number_format($stock_arr[$id]['mos'],2) . "' link=\"P-detailsWin,width=900,height=600,toolbar=no,scrollbars=yes, resizable=no-dashboard_wms_mos_drilldown.php?prov_sel=$province&from_date=$from_date&level=7&wh_id=$id&product=$product_array\"/>";
...
//        exit;
        ...
        echo renderChart(PUBLIC_URL . "FusionCharts/Charts/MSColumn2D.swf", "", $xmlstore, 'dhq_im_a1', '100%', 300, false, false);
        ?>
